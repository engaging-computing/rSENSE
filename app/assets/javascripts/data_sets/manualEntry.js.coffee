  ...
  if namespace.controller is "data_sets" and namespace.action is "manualEntry"
    ...
    settings =
      ...
      upload:
        ...
        error: (j, s, t) =>
              console.log [j, s, t]
              ($ '.mainContent').prepend "<div class='alert alert-danger alert-dismissable'><strong>An error occurred: </strong> #{t}</div>"
        success: (data, textStatus, jqXHR) ->
          ...
          if jdata.status == "unprocessable_entity"
            ...
            window.location = jdata.data.url
