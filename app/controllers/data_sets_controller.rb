    ...
    unless params['data'].nil?
      ...
      if sane[:status]
        ...
        if @data_set.save!
          ...
          ret = { status: :unprocessable_entity, error: @data_set.errors.full_messages }
        end
      else
        err_msg = sane[:status] ? dataset.errors.full_messages : sane[:msg]
        respond_to do |format|
          format.json { render json: { data: sane[:data_obj], error: err_msg }, status: :unprocessable_entity }
        end
      end
    end
    ...
    if sane[:status]
      ...
      respond_to do |format|
        if dataset.save
          ...
          format.json { render json: { error: dataset.errors.full_messages }, status: :unprocessable_entity }
        end
      end
    else
      err_msg = sane[:status] ? dataset.errors.full_messages : sane[:msg]
